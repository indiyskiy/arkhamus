package com.arkhamusserver.arkhamus.logic.ingame.loop.netty.responsemapper.lantern
...
import com.arkhamusserver.arkhamus.view.dto.netty.response.lantern.LightLanternNettyResponse
...
@Component
class LightLanternNettyResponseMapper(
    ...
    ): LightLanternNettyResponse {
        (requestProcessData as LightLanternRequestProcessData).let {
            val lanternStateInfo = it.lantern?.let { lantern ->
                LanternStateInfo(
                    costValue = 1,
                    costItem = Item.SOLARITE.id,
                    state = lantern.lanternState,
                    fuel = lantern.fuel,
                )
            }
            return LightLanternNettyResponse(
                lanternStateInfo = lanternStateInfo,
                successfullyLit = it.successfullyLit,
                tick = it.tick,
                userId = user.id!!,
