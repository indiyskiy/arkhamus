...
class SearchForSoundAbilityCast(
    ...
    override fun cast(
        ability: Ability, abilityRequestProcessData: AbilityRequestProcessData, globalGameData: GlobalGameData
    ): Boolean {
    ...
    override fun cast(
        sourceUser: InGameGameUser, ability: Ability, target: WithStringId?, globalGameData: GlobalGameData
    ): Boolean {
        val user = sourceUser
        user.stateTags += UserStateTag.INVESTIGATING
        val visibilityModifier = resolver.toVisibilityModifier(ability)
